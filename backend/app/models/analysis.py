...
class AnalysisResult(Base):
    ...
    # 트릭 분류 결과
    tricks_detected: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    ...
    # 피드백 (리스트로 저장)
    feedback_text: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)

    # 캐릭터 애니메이션 경로
    animation_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    highlight_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    overlay_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)

    # 포즈 데이터 (프레임별 관절 좌표)
    pose_data_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
